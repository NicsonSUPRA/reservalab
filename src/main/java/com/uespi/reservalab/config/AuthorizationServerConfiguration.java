...
public class AuthorizationServerConfiguration {
    ...
    @Bean
    OAuth2TokenCustomizer<JwtEncodingContext> jwTokenCustomizer() {
        return context -> {
            var principal = context.getPrincipal();

            if (principal instanceof CustomAuthentication) {
                CustomAuthentication customAuthentication = (CustomAuthentication) principal;

                OAuth2TokenType tokenType = context.getTokenType();

                if (tokenType.equals(OAuth2TokenType.ACCESS_TOKEN)) {
                    context.getClaims()
                            .claim("nome", customAuthentication.getUsuario().getNome())
                            .claim("authorities", customAuthentication.getUsuario().getAuthorities());
                }
            }
        };
    }
